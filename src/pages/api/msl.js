...
export default async function handler(req, res) {
    ...
    if (req.method === 'GET') {
        try {
            const prefix = `${region.toLowerCase()}/msl_`; // Ensure the region folder is lowercase

            cloudinary.v2.api.resources(
                {
                    type: 'upload',
                    prefix, // Assuming all relevant images start with '{region}/msl_'
                ...
                function (error, result) {
                    ...
                    result.resources.forEach(resource => {
                        const parts = resource.public_id.match(/^.*msl_(\d{2}-\d{2}-\d{4})_(00UTC|12UTC)_(\d{2}-\d{2}-\d{4})$/);
                        if (!parts) return;
