...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    try {
      const prefix = `${region.toLowerCase()}/wind_gust_10m_`; // Ensure the region folder and date are formatted correctly
      ...
      result.resources.forEach((resource) => {
        const parts = resource.public_id.match(
          /^.*wind_gust_10m_(\d{2}-\d{2}-\d{4})_(00UTC|12UTC)_(\d{2}-\d{2}-\d{4})$/i
        );
