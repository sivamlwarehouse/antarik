...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    try {
      const prefix = `${region.toLowerCase()}/severe_weather_index_`; // Ensure the region folder and date are formatted correctly
      ...
      result.resources.forEach((resource) => {
        const parts = resource.public_id.match(
          /^.*severe_weather_index_(\d{2}-\d{2}-\d{4})_(00UTC|12UTC)_(\d{2}-\d{2}-\d{4})$/i
        );
