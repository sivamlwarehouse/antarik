...
export default async function handler(req, res) {
    const { date, region } = req.query; // Expected format: '27-05-2024' and region

    if (req.method === 'GET') {
        try {
            const prefix = `${region}/total_precipitation_`; // Adjust the prefix to include the region folder

            cloudinary.v2.api.resources(
                {
                    type: 'upload',
                    prefix, // Assuming all relevant images start with '{region}/total_precipitation_'
                ...
                function (error, result) {
                    ...
                    result.resources.forEach(resource => {
                        const parts = resource.public_id.match(/^.*total_precipitation_(\d{2}-\d{2}-\d{4})_(00UTC|12UTC)_(\d{2}-\d{2}-\d{4})$/);
                        if (!parts) return;
